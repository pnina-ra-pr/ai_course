...
class ChatHost:
    def __init__(self):
        self.mcp_clients: list[MCPClient] = [MCPClient("./func.py")]
    ...
    async def process_query(self, query: str) -> str:
        ...
        while True:
            response = self.anthropic.messages.create(
                ...
                system= """
אתה נותן מענה למשתשמ ששואל בצ'אט מה מזג האויר בעיר מסויימת בישראל
1. אתה מקבל מהמשתמש את שם העיר
2. מנווט לאתר מזג האויר
3. מזין את שם העיר שהמשתמש ביקש
4. מקליק על שם העיר בתיבת הסלקט
5.חכה לטעינת העמוד מוודא דאכן נפתח העמוד הנכון
6. מחזיר תשובה למשתמש מה מזג האויר רק ע''פ המידע שיש לך באתר"""
            )
