...
- **Studio** — generate artifacts: **Infographic · Summary · FAQ** (each its own standalone
  agent; the result opens in the viewer and is saved as a note). **PowerPoint** is still
  pending — it needs a `.pptx` generation skill and a file download, not just structured
  output.
...
src/
  agents/               one agent per feature
    ...
    studio.py           the artifact generators (structured output, one schema each)
  core/                 config, source ingestion, the live SourceStore
...
generators are standalone, stateless agents (invoked directly by the Studio, not part of
the chat): no tools and no memory, every active source handed over verbatim, and a
`response_format` so the answer comes back as a validated Pydantic model rather than free
text. Each schema renders itself to markdown, and the result is saved as a note.
Capabilities not built yet return `501` and the UI shows a "coming soon" notice.
...
| Structured output (Studio artifacts) | ✅ done — `agents/studio.py` (PowerPoint still pending) |
